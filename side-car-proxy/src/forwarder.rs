...
pub async fn forward(
    ...
    rules: Vec<(PathFragments, std::time::Duration)>,
    ...
    let (res, cx): (Result<Response<BoxBody<Bytes, error::ProxyError>>>, Context) = {
        ...
        let ok = match &res {
            Ok(resp) => {
                ...
                if (400u16..500u16).contains(&s.as_u16()) {
                    warn!("Client error from upstream {}: {}", ep.authority, s);
                }
                s.is_success() || !(400u16..500u16).contains(&s.as_u16())
            }
            Err(_) => {
                warn!("Request error to {}", ep.authority);
                false
            }
        };
...
pub async fn forward_mesh(
    ...
    let (res, cx): (Result<Response<BoxBody<Bytes, error::ProxyError>>>, Context) = {
        ...
        if let Some(dest) = authority_from(&parts) {
            let uri = http::Uri::builder()
                .scheme("https")
                .authority(dest)
                .path_and_query(
